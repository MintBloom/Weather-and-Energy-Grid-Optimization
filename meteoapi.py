...
def read_demand_data_from_csvfile(file_path): # file_path is the path to the CSV file containing demand data
    ...
    for entry in obj:
        if entry.is_file() and entry.name.endswith('.csv'):
            ...
            df["SETTLEMENT_DATE"] = pd.to_datetime(df["SETTLEMENT_DATE"], format='mixed', dayfirst=True)  # convert the "SETTLEMENT_DATE" column to datetime format
...
if __name__ == "__main__":
    historic_weather_df = get_historic_weather_data(51.5085, -0.1257, "2025-06-20", "2026-06-20")
